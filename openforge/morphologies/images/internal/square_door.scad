...
module square_door_hole(x=2, y=2, border=0, wall=10.2, base=false, external=false) {
    ...
    if (x < 3) {
        color("DarkGrey") difference() {
            ...
            translate([0,_basis*y-(12.7-wall),base_buffer]) union() {
        ...
        difference() {
            children(1);
            translate([0,_basis*y-(12.7-wall)-sub,base_buffer]) color("DarkGrey") translate([_basis/2+1.7,6.2,1]) cylinder(50,1,1,$fn=25);
            translate([0,_basis*y-(12.7-wall)-sub,base_buffer]) color("DarkGrey") translate([2*_basis-(_basis/2+1.7),6.2,1]) cylinder(50,1,1,$fn=25);
        }
    } else {
        move = (x-2) * 12.5;
        color("DarkGrey") difference() {
            children(0);
            translate([-move,_basis*y-(12.7-wall),base_buffer]) union() {
                translate([_basis*2-(29+28)/2,-_basis/2-6,0]) cube([29+28,_basis+3,51.2]);
                translate([_basis*2-(33+28)/2,-_basis/2-2,_basis*2-6-4.1]) cube([33+28,_basis+3,4.2]);
                translate([_basis*2-(25+28)/2,-1,0]) cube([25+28,18.7,51.2]);
            }
        }
        difference() {
            children(1);
            translate([-move,_basis*y-(12.7-wall)-sub,base_buffer]) color("DarkGrey") translate([_basis+0.4,6.2,1]) cylinder(50,1,1,$fn=25);
            translate([-move,_basis*y-(12.7-wall)-sub,base_buffer]) color("DarkGrey") translate([3*_basis-0.4,6.2,1]) cylinder(50,1,1,$fn=25);
        }
    }
}

module square_door(x=2, y=2, wall=10.2, base=false, external=false) {
    h = 2;
    base_buffer = base ? 6 : 0;
    
    square_door_hole(x=x, y=y, wall=wall, base=base, external=external) {
        color("Grey") {
            sub = external ? 0 : wall;
...
//square_door(x=2, y=2, wall=10.2, base=false, external=false);
